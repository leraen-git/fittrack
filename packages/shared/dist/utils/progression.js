...
export function calcExerciseVolume(sets) {
    return sets
...
export function getExerciseStatus(delta) {
    if (delta > PROGRESSION_THRESHOLD)
        return 'improved';
    if (delta < -PROGRESSION_THRESHOLD)
        return 'declined';
    return 'stable';
...
export function getTrend(last5sessions) {
...
export function getCoachingTip(exerciseName, trend, currentMax) {
    switch (trend) {
        case 'improving':
